<template>
    <div class="container-sell">
                ...
                <div class="filter-left flex gap-8">
                    <div class="list-search">
                        <BaseInput showIcon="true" v-model="keyword" inputClass="m-input"
                            placeholder="Tìm theo tên cửa hàng, địa chỉ" @keypress.enter="handleSearch"
                            :clearInput="true" @clearInput="handleSearch">
                        </BaseInput>
                    </div>
                </div>
                <div class="filter-right">
                    <div class="cart-button" @click="addStore">
                        Thêm cửa hàng
                    </div>
                </div>
            </div>
        ...
        <StorePageDetail v-if="showDetail" :modeAdd="modeAdd" @closePopup="closePopup"></StorePageDetail>
        <!-- Modal xác nhận xóa -->
...
import StorePageDetail from './StorePageDetail.vue';
...
const storeDelete = ref(null);

const deleteStore = (product) => {
    showDeleteModal.value = true;
    storeDelete.value = product;
};

const closeDeleteModal = () => {
    showDeleteModal.value = false;
    storeDelete.value = null;

};

const confirmDeleteProduct = async () => {
    if (storeDelete.value) {
        // Gọi hàm xóa sản phẩm trong store hoặc thực hiện hành động xóa
        await store.dispatch('deleteStore', storeDelete.value.StoreID);
        store.dispatch('getStore');
    }
